...
def show_hero(hero):
    ...
    print(f'Оружие: {hero["weapon"]}')
    print(f'Доспехи: {hero["armor"]}')
    print(f'Щит: {hero["shield"]}')
...
def show_inventory(hero: dict) -> None:
    os.system("cls")
    print("Инвентарь:\n")
    for num, option in enumerate(hero["inventory"]):
        print(f"{num}.{option}")
    if not hero["inventory"]:
        print("Пустой\n")
    else:
...
def equip_item(hero: dict) -> None:
    if not hero["inventory"]:
        ...
        if idx is not None:
            if idx <= len(hero["inventory"]) - 1 and idx > -1:
                if hero["inventory"][idx] == "sword":
                    if hero["weapon"] is True:
                        hero["inventory"].append(hero["weapon"])
                        hero["weapon"] = "sword"

def show_item(item: dict) -> None:
    if item['тип'] == 'оружие':
        f"{item['название']}, {item['атака']}"
    elif item['тип'] == 'щит':
        f"{item['название']}, {item['защита']}"
